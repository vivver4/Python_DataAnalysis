...
read_csv所返回的这个TextParser对象使你可以根据chunksize对文件进行逐块迭代（这里如果用read_table的话注意要加sep='\s+'，否则列标签会出问题，另外piece['key']会返回Series，再用add方法会自动叠加相同的部分）。比如说，我们可以迭代处理ex6.csv，将值计数聚合到"key"列中，如下所示：
```python
